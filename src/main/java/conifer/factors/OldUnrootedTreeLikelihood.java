 ...
 */
public class OldUnrootedTreeLikelihood<R extends CTMCParameters> implements GenerativeFactor
{
  /**
   * 
   */
  ...
  private void addInitialDistribution(DiscreteFactorGraph<TreeNode> result, TreeNode arbitraryRoot, int nSites, CTMC ctmc)
  {
    ...
    result.unaryTimesEqual(arbitraryRoot, new SimpleMatrix(allStatios));
  }
   ...
   */
  public static OldUnrootedTreeLikelihood<JukeCantorRateMatrix> fromObservations(File inputFile)
  {
    ...
    return new OldUnrootedTreeLikelihood<JukeCantorRateMatrix>(randomTree, defaultRateMatrix(factory.nSymbols()), leafLikelihoods, nSites);
  }
  ...
  public static OldUnrootedTreeLikelihood<JukeCantorRateMatrix> createEmptySyntheticNucleotideLikelihood(int nSites, List<TreeNode> leaves)
  {
    Random rand = new Random(1);
    UnrootedTree randomTree = NonClockTreePrior.generate(rand, Exponential.on(RealVariable.real()), leaves);
    return new OldUnrootedTreeLikelihood<JukeCantorRateMatrix>(randomTree, defaultRateMatrix(4), new HashMap<TreeNode, UnaryFactor<TreeNode>>(), nSites);
  }
  ...
  private OldUnrootedTreeLikelihood(UnrootedTree tree, R rateMatrix,
